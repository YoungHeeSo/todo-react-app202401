...
const TodoInput = ({ onAdd }) => {
    ...
    const submitHandler = e => {
        ...
        e.preventDefault(); // 리액트는 비동기식 이므로 페이지가 새로고침 되는 것을 막아주는게 좋다

        // 폼이 제출되면 입력창 비우기
        setTodoText(''); // 값 지정뿐 아니라 렌더링까지 해주어야 input을 빈 값으로 만들어준다 그러므로 상태변수 사용함
    }
            ...
            {open && (<div className='form-wrapper'>
                <form className='insert-form' onSubmit={submitHandler}> {/*submit 버튼을 누르거나 엔터를 누를 때 실행*/}
                    <input
                        ...
                        onChange={todoChangeHandler} // input 의 경우 change event more good
                        value={todoText}
                    />
                </form>
            </div>)}
